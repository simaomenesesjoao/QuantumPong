...
void Server::addEventQueue(event_queue *eq){
    eventQueue = eq;
}
...
void Server::processPressedKey(uint8_t *data){
    ...
    if(S_ON_HANDLER){std::cout << "p" << player_number << " on_pressed_key\n";}
    if(S_ON_HANDLER){std::cout << "key" << keycode << "\n";}
...
void Server::GameOffHandler(uint8_t* data){
    if(S_ON_ENTRY) std::cout << "Server::GameOffHandler\n";
...
void Server::GameRunningOnEntry(){
    if(S_ON_ENTRY) std::cout << "Server::GameRunningOnEntry\n";
    state = GameRunning;
    stateString = "GameRunning";
    activeHandler = &Server::GameRunningHandler;
...
void Server::GameRunningHandler(uint8_t* data){
    if(S_ON_HANDLER) std::cout << "Server::GameOnHandler\n";
    Event<EV_GENERIC> evg(data);
...
void Server::GamePausedOnEntry(){
    if(S_ON_ENTRY) std::cout << "GamePausedOnEntry\n";

    activeHandler = &Server::GamePausedHandler;    
    state = GamePaused;
    stateString = "GamePaused";
    
}
void Server::GamePausedHandler(uint8_t* data){
    if(S_ON_HANDLER) std::cout << "GamePausedHandler\n";
    Event<EV_GENERIC> evg(data);
